# 目录

- [前言](./foreword/README.md)
  - [本书项目结构](./foreword/about.md)
  - [关于作业区和软件篋实例](./foreword/crate.md)
  - [关于Cargo工具基础命令](./foreword/cargo.md)
  - [Rust语言新思维和新概念](./foreword/rust.md)
- [软件篋hello_exercism](./hello-world/README.md)
  - [关于软件篋项目](./hello-world/about.md)
  - [共享篋：开发命令](./hello-world/commands-lib.md)
  - [共享篋：程序代码解释](./hello-world/lib.md)
  - [共享篋：目录tests的测试代码解释](./hello-world/lib-tests.md)
  - [共享篋：目录src的测试代码解释](./hello-world/src-tests.md)
  - [本地程序：开发命令](./hello-world/commands-local.md)
  - [本地程序：程序代码解释](./hello-world/bin-local.md)
  - [仓库程序：开发命令](./hello-world/commands-bin.md)
  - [仓库程序：程序代码解释](./hello-world/bin.md)
  - [题外话](./hello-world/off-topic.md)
- [软件篋trait_exerci](./hello-trait/README.md)
  - [类型关键词`struct`](./hello-trait/examples-struct.md)
  - [关键词`impl`与方法代码实现](./hello-trait/examples-impl.md)
  - [函数与方法代码实现](./hello-trait/examples-methods.md) 
  - [衔接关键词trait](./hello-trait/examples-trait.md)
  - [特质实现及其对象](./hello-trait/examples-trait-impl.md)
  - [题外话：默认衔接特质Default](./hello-trait/examples-trait-default.md)
  - [程序代码结构](./hello-trait/lib-structure.md)
  - [关于软件篋项目](./hello-trait/about.md)
  - [共享篋：开发命令](./hello-trait/commands-lib.md)
  - [共享篋：程序代码解释](./hello-trait/lib-codes.md)
  - [共享篋：目录测试代码解释](./hello-trait/lib-tests.md)
  - [题外话](./hello-trait/off-topic.md)
- [软件篋mod_trait_exerci](./hello-mod-trait/README.md)
  - [关于软件篋mod_trait_exerci](./hello-mod-trait/about.md)
  - [文件与模块](./hello-mod-trait/crate-structure.md)
  - [共享篋：程序结构和代码解释](./hello-mod-trait/lib-codes.md)
  - [三种调用方式解释](./hello-mod-trait/exampels-codes.md)
  - [理解动态与静态调度实现](./hello-mod-trait/examples-dispatch.md)
  - [深度解析动态与静态调度实现](./hello-mod-trait/examples-miri.md)
  - [共享篋：简单三层结构实现](./hello-mod-trait/lib-fn.md)
  - [共享篋：基于封装的静态调度实现](./hello-mod-trait/lib-static.md)
  - [共享篋：基于封装的动态调度实现](./hello-mod-trait/lib-dynamic.md)
  - [共享篋：单元测试代码解释](./hello-mod-trait/lib-tests.md)
  - [题外话：泛型实例](./hello-mod-trait/off-topic.md)
- [软件篋borrowing_exerci](./hello-borrowing/README.md)
  - [关于应用篋borrowing_exerci](./hello-borrowing/about.md)
  - [特质：克隆（Clone）和复制（Copy）](./hello-borrowing/clone-copy.md)
  - [引用`Reference`与指针`Pointer`基本概念](./hello-borrowing/reference-pointer.md)
  - [引用、类型与原始指针解释](./hello-borrowing/reference-type-pointer.md)
  - [应用篋：简单借用方法](./hello-borrowing/borrowing-simple.md)
  - [应用篋：移动关键词`move`](./hello-borrowing/keyword-move.md)
  - [应用篋：引用借用实例](./hello-borrowing/borrowing-reference.md)
  - [应用篋：指针借用实例](./hello-borrowing/borrowing-pointer.md)
  - [应用篋：固定大小类型实例](./hello-borrowing/borrowing-fixed-size.md)
  - [应用篋：可变大小类型实例](./hello-borrowing/borrowing-re-sizable.md)
  - [应用篋：栈（Stack）和堆（Head）](./hello-borrowing/stack_head.md)
  - [应用篋：函数借用方法](./hello-borrowing/borrowing-fn.md)
  - [题外话：应用篋的软件包结构](./hello-borrowing/bin-lib.md)
  - [题外话：Cargo工具features功能](./hello-borrowing/cargo-features.md)
  - [题外话：介绍几个软件篋](./hello-borrowing/crates.md)
- [软件篋deref_exerci](./hello-deref/README.md)
  - [关于软件篋deref_exerci](./hello-deref/about.md)
  - [开发软件篋deref_exerci](./hello-deref/develop.md)
  - [使用软件篋deref_exerci](./hello-deref/apply.md)
- [软件篋macro_exerci](./hello-macro/README.md)
  - [关于软件篋macro_exerci](./hello-macro/about.md)
  - [开发软件篋macro_exerci](./hello-macro/develop.md)
  - [使用软件篋macro_exerci](./hello-macro/apply.md)
- [软件篋generics_exerci](./hello-generics/README.md)
  - [关于软件篋generics_exerci](./hello-generics/about.md)
  - [开发软件篋generics_exerci](./hello-generics/develop.md)
  - [使用软件篋generics_exerci](./hello-generics/apply.md)

